...
def metric(scores, targets, types):
    ...
    for index in range(num_class): #For each class 
        score = scores[:, index]
        ...
        gt_num[index] = np.sum(target == 1) #all values with 1
        predict_num[index] = np.sum(score >= 0.5) 
        tp_num[index] = np.sum(target * (score >= 0.5))

        ######################################################
        precision = tp_num[index] / predict_num[index] if predict_num[index] != 0 else 0
        recall = tp_num[index] / gt_num[index] if gt_num[index] != 0 else 0
    ...
    #added precision_list, recall_list, f1_list
    return OP, OR, OF1, CP, CR, CF1, precision_list, recall_list, f1_list
