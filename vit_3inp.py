...
class VIT_CSRA(nn.Module): #defines the hybrid embedding layer that accepts both img patches and feature maps from cnn input
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.classifier = MHA(input_dim=1536, num_heads=cls_num_heads, num_classes=cls_num_cls, lam=lam)
    ...
    def forward_train(self, x1, x2, target):

        # print(x.shape)
        # print("Hello")
        x1 = self.backbone(x1) #gets feature map
        x2 = self.backbone(x2)
    ...
    def forward(self, x1, x2, target=None):
        if target is not None:
            return self.forward_train(x1, x2, target)
        else:
            return self.forward_test(x1, x2)
